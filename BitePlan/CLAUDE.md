...
```bash
# Build
xcodebuild -project BitePlan.xcodeproj -scheme BitePlan \
  -destination 'platform=iOS Simulator,name=iPhone 16' build

# Run all tests
xcodebuild test -project BitePlan.xcodeproj -scheme BitePlan \
...
## Safari Share Extension (BitePlanShare target)

`BitePlanShare/ShareViewController.swift` — a Share Extension that lets users
send a recipe URL from Safari directly to BitePlan.

### Flow
1. User taps Share in Safari → selects BitePlan
...
4. Extension walks the responder chain to open `biteplan://import` (URL scheme
   registered on the BitePlan target — do not use `UIApplication.shared` or
...
5. Main app receives the URL via `.onOpenURL` → `consumePendingRecipeURL()` in
   `BitePlanApp` posts a `Notification.Name.bitePlanImportURL` notification
