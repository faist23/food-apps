...
extension Notification.Name {
    static let bitePlanImportURL = Notification.Name("bitePlanImportURL")
}

// E-2: App store error types for graceful failure handling.
enum BitePlanError: LocalizedError {
...
private struct BitePlanErrorView: View {
    let error: Error
    let onRetry: () -> Void
...
struct BitePlanApp: App {
            ...
            Group {
                if let error = storeError {
                    ...
                    BitePlanRootView()
                        ...
                        .onOpenURL { url in
                            // Launched via biteplan://import from the Share Extension.
                            // The extension already wrote the URL to shared UserDefaults.
                            guard url.scheme == "biteplan", url.host == "import" else { return }
    ...
    private func loadContainer() {
        ...
        guard let containerURL = FileManager.default
            ...
            storeError = BitePlanError.appGroupNotConfigured(groupID)
            return
        }
        let storeURL = containerURL.appendingPathComponent("biteledger.store")
        do {
            let schema = Schema(versionedSchema: BitePlanSchemaV4.self)
